...
auto main(const int argc, const char * const * const argv) -> int
{
    ...
    if (vm.count("output-file")) {
        cond_set_field<std::int16_t>(conf.active_pixel_width, vm, "active-pixel-width");
        cond_set_field<std::int16_t>(conf.active_pixel_height, vm, "active-pixel-height");
        cond_set_field<std::int16_t>(conf.pixel_array_width, vm, "pixel-array-width");
        cond_set_field<std::int16_t>(conf.pixel_array_height, vm, "pixel-array-height");
        cond_set_field<std::int32_t>(conf.sensor_timing, vm, "sensor-timing");
        cond_set_field<std::int32_t>(conf.output_timing, vm, "output-timing");
        cond_set_field<float>(conf.fps, vm, "fps");
        ...
        little_int32_t active_pixel_width = static_cast<std::int32_t>(conf.active_pixel_width);
        output_file.write(reinterpret_cast<std::fstream::char_type*>(&active_pixel_width), sizeof(active_pixel_width));
        little_int32_t active_pixel_height = static_cast<std::int32_t>(conf.active_pixel_height);
        output_file.write(reinterpret_cast<std::fstream::char_type*>(&active_pixel_height), sizeof(active_pixel_height));
        output_file.seekp(conf_offset);
        output_file.write(reinterpret_cast<std::fstream::char_type*>(&conf), sizeof(conf));
